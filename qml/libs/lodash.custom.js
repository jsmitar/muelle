.pragma library
.import "qrc:/polyfills/timer.js" as Timer
...
(function () {
  ...
  function baseGetTag(value) {
    ...
    return (symToStringTag && symToStringTag in Object(
              value)) ? getRawTag(value) : objectToString(value)
  }
  ...
  function getRawTag(value) {
    var isOwn = hasOwnProperty.call(value,
                                    symToStringTag), tag = value[symToStringTag]
   ...
   * until to the next tick, similar to `Timer.setTimeout` with a timeout of `0`.
   *
  ...
  function debounce(func, wait, options) {
    ...
    function leadingEdge(time) {
      ...
      timerId = Timer.setTimeout(timerExpired, wait)
      // Invoke the leading edge.
      return leading ? invokeFunc(time) : result
    }

    function remainingWait(time) {
      var timeSinceLastCall = time
          - lastCallTime, timeSinceLastInvoke = time
                          - lastInvokeTime, timeWaiting = wait - timeSinceLastCall
    ...
    function timerExpired() {
      ...
      // Restart the timer.
      timerId = Timer.setTimeout(timerExpired, remainingWait(time))
    }
