...
struct vtable
{
  ...
  explicit constexpr vtable() noexcept
    ...
    , copy_ptr{static_cast<copy_ptr_t>([](storage_ptr_t, storage_ptr_t) noexcept->void{})}
    , destructor_ptr{static_cast<destructor_ptr_t>([](storage_ptr_t) noexcept->void{})}
    , size{}
  {}

  template<typename C>
  explicit constexpr vtable(wrapper<C>) noexcept
    : invoke_ptr{static_cast<invoke_ptr_t>([](storage_ptr_t storage_ptr, Args &&... args) -> R {
      return (*static_cast<C *>(storage_ptr))(std::forward<Args>(args)...);
    })}
    , copy_ptr{static_cast<copy_ptr_t>([](storage_ptr_t dst_ptr, storage_ptr_t src_ptr) -> void {
      new (dst_ptr) C{(*static_cast<C *>(src_ptr))};
    })}
    , destructor_ptr{static_cast<destructor_ptr_t>(
        [](storage_ptr_t storage_ptr) noexcept->void { static_cast<C *>(storage_ptr)->~C(); })}
    , size{sizeof(C)}
  {}
...
template<typename R, typename... Args>
static const auto empty_vtable_ptr = std::addressof(empty_vtable<R, Args...>);
