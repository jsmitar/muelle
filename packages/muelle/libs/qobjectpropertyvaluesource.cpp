...
void QObjectPropertyValueSource::setTarget(const QQmlProperty &prop) {
  const auto signalIndex =
      mTargetProperty.property().notifySignal().methodIndex();
  const auto signalMethod =
      mTargetProperty.object()->metaObject()->method(signalIndex);

  QObject::disconnect(
      mTargetProperty.object(), signalMethod, this,
      QMetaMethod::fromSignal(&QObjectPropertyValueSource::targetValueChanged));

  mTargetProperty = prop;
  prop.connectNotifySignal(
      this,
      QMetaMethod::fromSignal(&QObjectPropertyValueSource::targetValueChanged)
          .methodIndex());
...
QVariant QObjectPropertyValueSource::initial() const { return mInitial; }
